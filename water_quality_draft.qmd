...
```{r}
# renaming sampling date column
# create new columns so we can easily extract year and month
# filter for years between 2007 and 2020
...
remove zip codes that don't have income and household data :(
```{r}
testing <- testing |> filter(!(ZIP %in% c("78667", "78680", "78691", "78712", "78763", "78767")))
```

```{r}
training <- training |> filter(!(ZIP %in% c("78667", "78680", "78691", "78712", "78763", "78767")))
```
